...
let expName = 'PAMIL_exp2';  // from the Builder filename that created this script
let expInfo = {
    'participant': `${util.pad(Number.parseFloat(util.randint(1, 90)).toFixed(0), 2)}`,
    'session': '001',
    'group': '',
};
...
flowScheduler.add(survey_InformDemRoutineBegin());
flowScheduler.add(survey_InformDemRoutineEachFrame());
flowScheduler.add(survey_InformDemRoutineEnd());
...
psychoJS.start({
  ...
  resources: [
    ...
    {'surveyId': 'a29d9256-f5c3-48ca-bec4-3f2b45837c27'},
...
async function experimentInit() {
  ...
  if (((Number.parseInt(expInfo["participant"]) % 3) === 0)) {
      expInfo["group"] = movie_groups[0];
  } else {
      if (((Number.parseInt(expInfo["participant"]) % 3) === 1)) {
          expInfo["group"] = movie_groups[1];
      } else {
          if (((Number.parseInt(expInfo["participant"]) % 3) === 2)) {
              expInfo["group"] = movie_groups[2];
          }
      }
  }
...
var frameN;
var continueRoutine;
var survey_InformDem;
var survey_InformDemClock;
function survey_InformDemRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'survey_InformDem' ---
    t = 0;
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    //--- Starting Routine 'survey_InformDem' ---
    survey_InformDem = new visual.Survey({
        win: psychoJS.window,
        name: 'survey_InformDem',
        surveyId: 'a29d9256-f5c3-48ca-bec4-3f2b45837c27',
    });
    survey_InformDemClock = new util.Clock();
    survey_InformDem.setAutoDraw(true);
    survey_InformDem.stauts = PsychoJS.Status.STARTED;
    survey_InformDem.isFinished = false;
    survey_InformDem.tStart = t;  // (not accounting for frame time here)
    survey_InformDem.frameNStart = frameN;  // exact frame index
    return Scheduler.Event.NEXT;
  }
}


function survey_InformDemRoutineEachFrame() {
  return async function () {
    t = survey_InformDemClock.getTime();
    frameN = frameN + 1;  // number of completed frames (so 0 is the first frame)
    // if survey_InformDem is completed, move on
    if (survey_InformDem.isFinished) {
      survey_InformDem.setAutoDraw(false);
      survey_InformDem.stauts = PsychoJS.Status.FINISHED;
...
function survey_InformDemRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'survey_InformDem' ---
    // get data from survey_InformDem
    const survey_InformDemResponse =  survey_InformDem.getResponse();
    for (const question in survey_InformDemResponse) {
      psychoJS.experiment.addData(`survey_InformDem.${question}`, survey_InformDemResponse[question]);
    }
    await survey_InformDem.save();
...
function AffectGrid_instructionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of AffectGrid_instructionComponents)
...
function PretestEmotion_valenceRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of PretestEmotion_valenceComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function PretestEmotion_arousalRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of PretestEmotion_arousalComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function EmotionVideoRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of EmotionVideoComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function GolbalLocal_instructionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of GolbalLocal_instructionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function GobalLocal_exampleRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of GobalLocal_exampleComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function example_feedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of example_feedbackComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function trialsGobalLocalLoopBegin(trialsGobalLocalLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisTrialsGobalLocal of trialsGobalLocal) {
...
function trialsMILLoopBegin(trialsMILLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisTrialsMIL of trialsMIL) {
...
function fixpointRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of fixpointComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function GobalLocal_FormalSessionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of GobalLocal_FormalSessionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function MLQPRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of MLQPComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function PostestEmotion_valenceRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of PostestEmotion_valenceComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function PostestEmotion_arousalRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of PostestEmotion_arousalComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
